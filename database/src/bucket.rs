...
use crate::{cache::ParKey, error::Error, partition::{Partition, PartitionEntry}, runtime::SendRuntime, unix_secs, Result};
...
impl Bucket {
    pub fn new_existing(partition: ActivePartition, live_window: Duration, path: PathBuf) -> Self {
        Self {
    ...
    pub async fn new<RT: SendRuntime>(path: PathBuf, now: u64, partition_map: &SlotMap<ParKey, Partition>) -> Result<Self> {
    ...
    pub async fn insert<RT: SendRuntime>(&self, entry_key: u128, value: Bytes, partition_map: &SlotMap<ParKey, Partition>) -> Result<(ParKey, PartitionEntry)>{ 
        let active = self.live_partition.load(Ordering::Relaxed);
        let guard: Guard<'_> = partition_map.pin();
        
        let (partition, key): (&Partition, ParKey) = if active.elapsed(unix_secs(), &self.live_window) {
            let key: ParKey = self.rotate::<RT>(partition_map).await?;
    ...
    async fn rotate<RT: SendRuntime>(&self, partition_map: &SlotMap<ParKey, Partition>) -> Result<ParKey> {
        let insertion_time = unix_secs();
        let part_path = self.path.join(format!("{}", insertion_time));
        let new_partition = Partition::new::<RT>(part_path).await?;
        let par_key = partition_map.insert(new_partition, &partition_map.pin());
        self.live_partition.store(ActivePartition::new(par_key, insertion_time), Ordering::Relaxed);
        Ok(par_key)
    }
}
